# jul/05/2025 10:27:59 by RouterOS 6.49.2
# software id = ERST-6AM2
#
# model = RouterBOARD 3011UiAS
# serial number = 8EED08A9740A
/interface bridge
add name=lo
/interface ethernet
set [ find default-name=ether1 ] comment=FO mac-address=CC:2D:E0:8A:D3:5F
...
/ip ipsec peer
add address=1.1.1.72/32 local-address=1.1.1.8 name=peer1
add address=1.1.1.71/32 local-address=1.1.1.8 name=peer2
/ip ipsec proposal
set [ find default=true ] enc-algorithms=aes-256-cbc,aes-128-cbc
/ip pool
add name=dhcp_pool0 ranges=172.18.8.31-172.18.8.125
/ip dhcp-server
...
add max-limit=64000/64000 name=queue3 target=172.17.8.104/32
add max-limit=128000/64000 name=queue200 target=172.18.8.109/32
/routing ospf area
set [ find default=true ] disabled=true
add area-id=0.0.0.1 name=area1
/routing ospf instance
set [ find default=true ] redistribute-connected=as-type-1 router-id=1.1.1.8
/user group
...
/ip address
add address=10.20.10.8/24 interface=ether1 network=10.20.10.0
add address=10.20.11.8/24 interface=ether2 network=10.20.11.0
add address=172.18.8.126/25 interface=ether3 network=172.18.8.0
add address=1.1.1.8 interface=lo network=1.1.1.8
add address=172.17.8.190/26 disabled=true interface=ether6 network=\
    172.17.8.128
add address=172.17.8.254/26 disabled=true interface=ether7 network=\
    172.17.8.192
add address=10.10.11.210/24 disabled=true interface=ether2 network=10.10.11.0
add address=192.168.1.12/24 interface=ether2 network=192.168.1.0
/ip dhcp-server network
add address=172.18.8.0/25 dns-server=192.168.99.37 domain=adrmm.co.mz \
    gateway=172.18.8.126
/ip dns
...
/ip ipsec identity
add peer=peer1 secret="\$2018\$"
add peer=peer2 secret="\$2018\$"
/ip ipsec policy
add dst-address=1.1.1.71/32 peer=peer2 src-address=1.1.1.8/32 tunnel=true
add disabled=true dst-address=1.1.1.72/32 peer=peer1 src-address=1.1.1.8/32 \
    tunnel=true
add disabled=true dst-address=172.16.0.0/22 peer=peer2 src-address=\
    172.17.8.0/24 tunnel=true
add dst-address=192.168.0.0/17 peer=peer2 src-address=172.18.8.0/25 tunnel=\
    true
add disabled=true dst-address=192.168.0.0/18 peer=peer2 src-address=\
    172.17.8.0/25 tunnel=true
/ip service
...
add area=area1 range=172.17.8.0/24
...
add area=area1 network=1.1.1.8/32
