# jul/05/2025 10:28:30 by RouterOS 6.43.4
# software id = E9UR-8XDH
#
# model = RouterBOARD 3011UiAS
# serial number = 8EED08B2D31E
/interface bridge
add name=lo
/interface ethernet
...
/ip pool
add name=dhcp_pool0 ranges=172.18.18.31-172.18.18.125
/ip dhcp-server
...
add max-limit=3000000/3000000 name=queue2 target=ether2
/routing ospf area
...
set [ find default=true ] redistribute-connected=as-type-1 \
    redistribute-static=as-type-1 router-id=1.1.1.18
...
/ip address
add address=10.20.10.18/24 interface=ether1 network=10.20.10.0
add address=10.20.11.18/24 interface=ether2 network=10.20.11.0
add address=172.18.18.126/25 interface=ether3 network=172.18.18.0
add address=1.1.1.18 interface=lo network=1.1.1.18
add address=172.17.18.190/26 disabled=true interface=ether6 network=\
    172.17.18.128
add address=172.17.18.254/26 disabled=true interface=ether7 network=\
    172.17.18.192
add address=10.10.11.210/24 disabled=true interface=ether2 network=10.10.11.0
add address=10.20.13.18/24 disabled=true interface=ether2 network=10.20.13.0
/ip dhcp-server network
add address=172.18.18.0/25 dns-server=192.168.99.37 domain=adrmm.co.mz \
    gateway=172.18.18.126
/ip dns
...
/ip ipsec peer
add address=1.1.1.72/32 disabled=true local-address=1.1.1.18 secret=\
    "\$2018\$"
add address=1.1.1.71/32 local-address=1.1.1.18 secret="\$2018\$"
/ip ipsec policy
add dst-address=1.1.1.71/32 sa-dst-address=1.1.1.71 sa-src-address=1.1.1.18 \
    src-address=1.1.1.18/32 tunnel=true
add disabled=true dst-address=1.1.1.72/32 sa-dst-address=1.1.1.72 \
    sa-src-address=1.1.1.18 src-address=1.1.1.18/32 tunnel=true
add disabled=true dst-address=172.16.0.0/22 sa-dst-address=1.1.1.71 \
    sa-src-address=1.1.1.18 src-address=172.17.18.0/24 tunnel=true
add dst-address=192.168.0.0/17 sa-dst-address=1.1.1.71 sa-src-address=\
    1.1.1.18 src-address=172.18.18.0/25 tunnel=true
add disabled=true dst-address=192.168.0.0/18 sa-dst-address=1.1.1.71 \
    sa-src-address=1.1.1.18 src-address=172.17.18.0/25 tunnel=true
/ip service
...
add area=area1 range=172.17.18.0/24
...
add area=area1 network=1.1.1.18/32
