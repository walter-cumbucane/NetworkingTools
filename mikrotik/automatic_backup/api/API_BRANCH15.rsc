# jul/05/2025 10:28:21 by RouterOS 6.43.4
# software id = 1X19-F926
#
# model = RouterBOARD 3011UiAS
# serial number = 8EED084A80D8
/interface bridge
add name=lo
/interface ethernet
set [ find default-name=ether1 ] comment=FO
set [ find default-name=ether2 ] comment=WIRELESS mac-address=\
    CC:2D:E0:92:3A:E0
...
/ip pool
add name=dhcp_pool0 ranges=172.18.15.31-172.18.15.125
/ip dhcp-server
...
add disabled=true max-limit=2000000/2000000 name=queue2 target=\
    172.17.15.115/32
add disabled=true max-limit=2000000/1530000 name=queue3 target=\
    172.17.15.106/32
/routing ospf area
...
/ip address
add address=10.20.10.15/24 interface=ether1 network=10.20.10.0
add address=10.20.11.15/24 interface=ether2 network=10.20.11.0
add address=172.18.15.126/25 disabled=true interface=ether3 network=\
    172.18.15.0
add address=1.1.1.15 interface=lo network=1.1.1.15
add address=172.17.15.190/26 disabled=true interface=ether6 network=\
    172.17.15.128
add address=172.17.15.254/26 disabled=true interface=ether7 network=\
    172.17.15.192
add address=10.10.11.210/24 interface=ether2 network=10.10.11.0
add address=172.18.15.126/25 interface=ether3 network=172.18.15.0
/ip dhcp-server network
add address=172.18.15.0/25 dns-server=192.168.99.37 domain=adrrm.co.mz \
    gateway=172.18.15.126
/ip dns
...
/ip ipsec peer
add address=1.1.1.72/32 local-address=1.1.1.15 secret="\$2018\$"
add address=1.1.1.71/32 local-address=1.1.1.15 secret="\$2018\$"
/ip ipsec policy
add dst-address=1.1.1.71/32 sa-dst-address=1.1.1.71 sa-src-address=1.1.1.15 \
    src-address=1.1.1.15/32 tunnel=true
add disabled=true dst-address=1.1.1.72/32 sa-dst-address=1.1.1.72 \
    sa-src-address=1.1.1.15 src-address=1.1.1.15/32 tunnel=true
add disabled=true dst-address=172.16.0.0/22 sa-dst-address=1.1.1.71 \
    sa-src-address=1.1.1.15 src-address=172.17.15.0/24 tunnel=true
add dst-address=192.168.0.0/17 sa-dst-address=1.1.1.71 sa-src-address=\
    1.1.1.15 src-address=172.18.15.0/25 tunnel=true
add disabled=true dst-address=192.168.0.0/18 sa-dst-address=1.1.1.71 \
    sa-src-address=1.1.1.15 src-address=172.17.15.0/25 tunnel=true
/ip service
...
add area=area1 range=172.17.15.0/24
...
add area=area1 network=1.1.1.15/32
add area=area1 network=10.20.10.0/24
add area=area1 network=10.20.11.0/24
/routing rip
set redistribute-connected=true redistribute-ospf=true redistribute-static=\
    true
